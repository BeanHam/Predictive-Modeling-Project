...
ols = lm(logprice ~ (dealer + school_pntg + diff_origin + artistliving + endbuyer + authorstyle + 
                     ...
                     + portrait + discauth + still_life + year + author)^2, 
         data = paintings_train_2)
...
```{r}
ols.2 = lm(logprice ~ Shape + school_pntg + author + subject + year*author + year*dealer + year*artistliving + dealer*Interm + dealer*Surface + dealer*paired + dealer*finished + diff_origin*Surface + diff_origin*portrait + artistliving*endbuyer + Interm*Surface + Interm*lrgfont + Surface*lrgfont + Surface*still_life + Surface*discauth + prevcoll*finished + paired*lrgfont + paired*discauth + diff_origin*authorstyle + diff_origin*still_life + finished*discauth + lrgfont*discauth + artistliving*finished + Interm*portrait + dealer*artistliving + authorstyle*prevcoll, data = paintings_train_2)
...
rf = randomForest(logprice ~ . -price,
                       data = paintings_train_2, 
                       importance = TRUE)
rf.pred = predict(rf, newdata = paintings_test_2)
```
