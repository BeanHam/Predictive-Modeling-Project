...
author=paintings_train %>% dplyr::select(author,logprice) %>% group_by(author) %>% summarise(ave=mean(logprice)) %>% mutate(authorlevel=sapply(ave, evaluate)) %>% dplyr::select(-ave)
