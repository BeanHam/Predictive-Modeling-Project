...
#length(unique(paris_paintings$subject, paris_paintings$dealer))
#
#unique(paris_paintings, margin = 1)
```
...
author_price_level = paintings_train %>% 
...
paintings_train=left_join(paintings_train,author_price_level,by="author")
paintings_test=left_join(paintings_test,author_price_level,by="author")  %>% mutate(authorlevel=ifelse(is.na(authorlevel),0,authorlevel))
...
```{r}
ols.2 = lm(logprice ~ Shape + school_pntg + prevcoll + Surface*diff_origin + dealer*Interm + dealer*paired + dealer*finished + diff_origin*portrait + artistliving*endbuyer + Interm*lrgfont + paired*lrgfont + paired*discauth + diff_origin*authorstyle + diff_origin*still_life + finished*discauth + lrgfont*discauth + artistliving*finished + Interm*portrait + dealer*artistliving, data = paintings_train_2)
