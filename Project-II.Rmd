...
#paris_paintings <- read.csv("~/STA521 - Fall2018/Final Project/paris_paintings.csv", comment.char="#", #stringsAsFactors=FALSE)
#
#uniquetest = unique(paintings_test, margin = 1)
#uniquedata = unique(paris_paintings, margin = 1)
```
...
author_price=paintings_train %>% 
   dplyr::select(author,logprice) %>% 
   group_by(author) %>% 
   summarise(ave=mean(logprice)) %>% 
   mutate(authorlevel=sapply(ave, evaluate)) %>% 
   dplyr::select(-ave)

paintings_train=left_join(paintings_train,author_price,by="author")
paintings_test=left_join(paintings_test,author_price,by="author")  %>% mutate(authorlevel=ifelse(is.na(authorlevel),0,authorlevel))
```
...
rf.train = paintings_train_2 %>% dplyr::select(-price, -author)
rf.test = paintings_test_2 %>% dplyr::select(-logprice, -price, -author)
