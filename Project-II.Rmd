...
ols.2 = lm(logprice ~ Shape + school_pntg + prevcoll
           + dealer*Interm + dealer*paired + dealer*artistliving
           + artistliving*endbuyer + artistliving*finished
           + diff_origin*Surface + diff_origin*portrait  + diff_origin*still_life + diff_origin*prevcoll
           + artistliving*year + artistliving*endbuyer
           + Interm*lrgfont
           + paired*lrgfont + paired*author + paired*subject + paired*discauth
           + finished*discauth
           + lrgfont*discauth, 
...
```{r}
ols.2 = lm(logprice ~ Shape + school_pntg + prevcoll + dealer*Interm + dealer*paired + dealer*finished + diff_origin*portrait + artistliving*endbuyer + Interm*lrgfont + Surface*diff_origin + paired*lrgfont + paired*discauth + diff_origin*authorstyle + diff_origin*still_life + finished*discauth + lrgfont*discauth + artistliving*finished + Interm*portrait + dealer*artistliving, data = paintings_train_2)
...
paris_paintings <- read.csv("~/STA521 - Fall2018/Final Project/paris_paintings.csv", stringsAsFactors=FALSE)
```

```{r}
paintings_test = paintings_test %>% 
   mutate(price = as.numeric(price))
paris_paintings = paris_paintings %>% 
   mutate(price = as.numeric(price))

paris_paintings = paris_paintings[,-1] %>% unique()

paris = paris_paintings %>% 
   dplyr::select(lot, position, dealer, price) %>% 
   unique() %>% 
   group_by(lot, position, dealer) %>% 
   summarise(price = mean(price, na.rm = TRUE))
paintings_test2 = left_join(paintings_test, paris, by=c("lot", "position", "dealer"))
paintings_test$price = paintings_test2$price.y
```
...
author=paintings_train %>% select(author,logprice) %>% group_by(author) %>% summarise(ave=mean(logprice)) %>% mutate(authorlevel=sapply(ave, evaluate)) %>% select(-ave)
paintings_train=left_join(paintings_train,author,by="author")
paintings_test=left_join(paintings_test,author,by="author")  %>% mutate(authorlevel=ifelse(is.na(authorlevel),0,authorlevel))
```
...
fitted = exp(predict(ols.2, newdata=paintings_test_2, type = "response"))
rmse(fitted, paintings_test$price)
