...
The overall characteristics of the model that we built in part I were: relatively lower bias, reasonable coverage, and high RMSE compared to other teams. The methodology used to arrive at the first model included EDA analyses, BAS, stepwise selection with AIC and BIC, which doesn't do an exhausive search for all possible models, thus the true model and the best model for prediction might not have been captured.
