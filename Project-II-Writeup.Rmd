---
title: "STA 521 Final Project Part II"
...
* dealer:Interm interaction: when an intermediary is present, which the price of the auctioned paintings differs significantly among different dealers. For instance, if the dealer is R and an intermediary is used, the average selling price is `107.0% higher` than when the dealer is J with an intermediary. This represents a difference in slope for dealer when an intermediary is involved.

* finished:discauth: given that the painting is noted for being highly finished, when the dealer engages with authenticity, the average price is expected to be `76.2% higher`. This represents a difference in slope.

* diff_origin:still_life: given that the origin of painting based on nationality of artist is different from the origin of painting based on dealer's classification, if the description indicates still life elements, the price is expected to be `-112.8% lower`. This represents a difference in slope for diff_origin when the painting is also a still life. 
...
```{r message=FALSE, warning=FALSE, echo = FALSE}
valid_pred = predictions = as.data.frame(
  exp(predict(ols.2, newdata=paintings_valid_2, type = "response")))

colnames(valid_pred) = c("prediction")
top_10 = data.frame(price = valid_pred$prediction, rownum = rownames(paintings_validation))

kable(top_10 %>% arrange(desc(price)) %>% top_n(n=10), digits = 1, align = "c")
```
...
compare_stats = data.frame(
  Adj_R2 = c(0.6129, 0.6634),
  RMSE = c(1502.99, 1219.52),
  Coverage = c(0.9506667, 0.9587)
)
rownames(compare_stats) = c("Part I Model", "Part II Model")
kable(compare_stats)
```
