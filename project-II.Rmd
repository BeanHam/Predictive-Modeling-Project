...
ols.test = paintings_test_2 %>% dplyr::select(-logprice)
...
rf = randomForest(logprice ~ .,
                  data = rf.train,
                  importance = TRUE,
                  mtry = round(sqrt(ncol(rf.train) -1), digits = 0),
                  maxnodes = 5)
...
xg.train = sparse.model.matrix(logprice ~ ., data = paintings_train_2)[,-1]
xg.response = paintings_train_2[,"logprice"]
boost = xgboost::xgboost(data = xg.train,
                         label = xg.response,
                         params = list(max_depth = 5, 
                                       gamma = 10, # threshold for split
                                       lambda = 0.01, 
                                       lambda_bias = 0.001),
...
xg.test = sparse.model.matrix(~ ., data = paintings_test_2[,-1])[,-1]
