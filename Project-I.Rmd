...
```{r}
paintings_train_3 = paintings_train_2 %>% 
  mutate(Interm = as.numeric(Interm))

micetest = mice::mice(paintings_train_3)
paintings_train_4 = mice::complete(output) %>% 
...
p = ggpairs(paintings_train_2, columns = c(1,20:39), 
            progress = FALSE, 
            title = "Title", cardinality_threshold = 60) +
  theme_bw(base_size = 10) + 
  theme(axis.text = element_text(size = 6))
