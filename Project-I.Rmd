...
```
\bftext
Use both JZS and g-prior to select variables. 
...
```{r}
ols.2 = lm(logprice ~ Shape + school_pntg + dealer*artistliving + dealer*Interm + dealer*Surface + dealer*paired + dealer*finished + dealer*discauth + diff_origin*Surface + diff_origin*portrait + artistliving*endbuyer + artistliving*authorstyle + Interm*Surface + Interm*lrgfont + Surface*lrgfont + Surface*still_life + Surface*discauth + prevcoll*finished + paired*lrgfont + paired*discauth + diff_origin*authorstyle + diff_origin*still_life + finished*discauth + lrgfont*discauth + artistliving*finished + authorstyle*prevcoll + Interm*portrait, data = paintings_train_2)

summary(ols.2)
par(mfrow = c(2,2))
plot(ols.2)
ols2.fitted = predict(ols.2, newdata = paintings_test)
rmse.ols2 = rmse(paintings_test$logprice, ols2.fitted)
```


```{r}

ols.3 = lm(logprice ~ Shape + school_pntg + dealer*Interm + dealer*Surface + dealer*paired + dealer*finished + dealer*discauth + diff_origin*Surface + diff_origin*portrait + artistliving*endbuyer + artistliving*authorstyle + Surface*lrgfont + Surface*still_life + Surface*discauth + prevcoll*finished + diff_origin*authorstyle + diff_origin*still_life + finished*discauth + lrgfont*discauth + artistliving*finished + Interm*portrait, data = paintings_train_2)

summary(ols.3)

par(mfrow = c(2,2))
plot(ols.3)
ols2.fitted = predict(ols.3, newdata = paintings_test)
rmse.ols3 = rmse(paintings_test$logprice, ols2.fitted)

## table
table_of_coef = exp(cbind(coef(ols.3), confint(ols.3)))
colnames(table_of_coef) = c("Coefficient", "2.5%", "97.5%")
kable(table_of_coef)
variable.names(ols.3)
```
