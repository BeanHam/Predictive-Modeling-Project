...
```{r yeehaw}
x_mat = as.matrix(train_df[,-1])
y_mat = as.matrix(train_df[,1])
glmnet::glmnet(y = y_mat, x = x_mat, family = c("gaussian"), alpha = 1)
```
...
bas.lm.inter = BAS::bas.lm(logprice ~ .^2, data = train_df, 
                          prior = "ZS-null",
                          alpha = 1,
                          method = "MCMC")
plot(bas.lm.inter, which = 4)
lm.inclProb = data.frame(inclProb = sort(summary(bas.lm.inter)[,1], decreasing = TRUE)) 
